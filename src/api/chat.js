...
import { redirectStore, reducerRedirect } from "./anyRedux";
    ...
    .on("value", (snapshot) => {
      ...
      for (let i in dataFire) {
        if (
          ...
          i.toLocaleLowerCase() !== user.login.toLowerCase()
        ) {
...
export function setMessage(user, text) {
  firebase
    ...
    .once("value", (snapshot) => {
      ...
      let isHaveId = dataFire.users[user.login].message.find(
        (elem) => elem[store.getState().login]
      );

      if (!isHaveId) {
        dataFire.users[user.login].message.push({
          [store.getState().login]: id,
        });
        dataFire.users[store.getState().login].message.push({
          [user.login]: id,
        });
        ...
        firebase
          .database()
          .ref(`/users/${user.login}/message`)
          .set(newDataUser);
        firebase
          .database()
          .ref(`/users/${store.getState().login}/message`)
          .set(newDataYour);
      }

      let messageId = dataFire.users[user.login].message.find(
        (elem) => elem[store.getState().login]
      );
      ...
      if (!isHaveMessage) {
        axios
          ...
          .then((res) => {
            let obj = {
              [`${new Date().getTime(res.data)}_${store.getState().login}`]:
                text,
            };
          ...
          if (+Object.keys(i)[0] === +Object.values(i2)[0]) {
            await getIdUser(Object.keys(i2)[0]).then((res) => {
              let newElem = {
                ...
                idUser: res,
                lastUpdate: {
                  text: Object.values(i[Object.keys(i)[0]])[
                    Object.values(i[Object.keys(i)[0]]).length - 1
                  ],
                  time: Object.keys(i[Object.keys(i)[0]])[
                    Object.values(i[Object.keys(i)[0]]).length - 1
                  ].split("_")[0],
                },
              };
...
export function watchElemMessage(
  info,
  setIdFirebaseMessage,
  setMessageWithUser
) {
