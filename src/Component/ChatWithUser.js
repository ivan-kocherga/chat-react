    ...
    return (
        <div className='chat'>
            <div>
                ...
                <main>
                    {
                                messageWithUser === 'initial' && info === 'initial'? <p className='main__noMessage'>Загрузка...</p> :
                                messageWithUser === 'initial' && info !== 'initial' ? <p className='main__noMessage'>Собщений нет</p> :
                                <div className='chat__main-messages'>
                                    {
                                        messageWithUser.map((elem, i) => {
                                            return (
                                                <div className={elem.user === info.login ? 'left' : 'right'} key={i}>
                                                    <div className="text">
                                                        <span>{elem.text}</span>
                                                    </div>
                    ...
                    <div className='chat__main-input'>
                        <input placeholder='Текст' value={inputText} onInput={e => setInputText(e.target.value)}/>
                        <button disabled={inputText.length === 0} onClick={createMessage}>Отправить</button>
                    </div>
                </main>
            </div>
